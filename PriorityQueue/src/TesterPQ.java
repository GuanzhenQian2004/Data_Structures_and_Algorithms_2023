...
public class TesterPQ {
    ...
    public static void main(String[] args) {
        ...
        PriorityQueue<Task> pq2 = new PriorityQueue<>(Collections.reverseOrder());


        for (int i =0; i < 800000; i++) {

            int rand = (int) (Math.random()*100000 + 1);
            Task toAdd = new Task(Integer.toString(rand), rand);

            pq.add(toAdd);
            pq2.add(toAdd);

            if (!pq.peek().equals(pq2.peek())) {
                System.out.println("run: " + i + "item: " + toAdd);
                System.exit(-1);
            }
        ...
        for (int i=0; i < 800000; i++) {
            if(!pq.remove().equals(pq2.remove())) {
                System.exit(-1);
            }
        }
